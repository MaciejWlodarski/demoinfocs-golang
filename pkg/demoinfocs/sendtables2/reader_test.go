...
// Huffman lookahead can leave complete bytes in bitVal; they still count as
// available input for a following binary field.
func TestReadBytesAfterLookahead(t *testing.T) {
	r := newReader([]byte{0x12, 0x34, 0x56})
	defer r.release()
	r.peekBits(24)
	assert.Equal(t, []byte{0x12, 0x34, 0x56}, r.readBytes(3))
	assert.Panics(t, func() { r.readBytes(1) })
}
